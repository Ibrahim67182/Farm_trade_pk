...
export async function GET(req: Request) {
  try {
    ...
    // Base conditions for user + purchase type
    ...
    // Search by commodity name, supplier name, or date
    if (search) {
      conditions.push(
        or(
    ...
    // Fetch paginated purchase transactions with supplier info
    const data = await db
      .select({
        id: transactions.id,
        ...
        supplierPhone: transactions.supplierPhone,
        supplierCompany: transactions.supplierCompany,
      })
      .from(transactions)
    ...
    console.error("❌ Error in fetching purchase transactions:", e);
