...
// for getting all suppliers data of a particular user
export async function GET(req: Request) {
  try {
    const userId = verifyAuth(req);

    if (!userId) {
      console.log("no user logged in to get suppliers data!");
      return NextResponse.json(
        { success: false, error: "user not found to get suppliers data!" },
        { status: 400 }
      );
    }
...
// creation of new supplier for a particular user
export async function POST(req: Request) {
  try {
    ...
    if (!userId || !name || !company || !email || !phone || !address) {
      console.log("Data fields are missing to create supplier record entry");
      return NextResponse.json(
        {
          success: false,
          error: "Data fields are missing to create supplier record entry",
        },
        { status: 400 }
      );
    }
    ...
    if (existing.length > 0) {
      return NextResponse.json(
        {
          success: false,
          error: "Email or phone already exists in record for another supplier!",
        },
        { status: 400 }
      );
    }
